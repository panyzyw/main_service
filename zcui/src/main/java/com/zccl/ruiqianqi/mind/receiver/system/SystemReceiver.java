...
public class SystemReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        // 进入监听
            ...
            String voiceFloatKey = context.getString(R.string.recycle_voice_float_key);
            String expressionKey = context.getString(R.string.recycle_expression_key);
            String from = intent.getStringExtra(fromKey);
            boolean isUseVoiceFloat = intent.getBooleanExtra(voiceFloatKey, true);
            boolean isUseExpression = intent.getBooleanExtra(expressionKey, true);
            sendStartListenEvent(from, isUseVoiceFloat, isUseExpression);
        }
        // 退出监听
        else if(ACT_STOP_LISTEN.equals(action)){
            String fromKey = context.getString(R.string.stop_from_key);
            sendStopListenEvent(intent.getStringExtra(fromKey));
        }
        // 进入视频监听
