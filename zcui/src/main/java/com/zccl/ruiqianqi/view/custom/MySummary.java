...
public class MySummary {
    ...
    /**
     * 1.对于最简单的情况，就是要被排序的类实现一个Comparable接口，然后实现compare方法，按照自然的方式进行减运算，
     *      返回减运算的结果，然后直接使用Collections.sort(List list)方法就行了。这一种叫做自然排序，只适合原始的List和Set。
     * 2.如果你不想使用自然排序，没问题，那么就定义一个Comparator对象，将逻辑在那里面去实现，
     *      然后使用Collections.sort(List list, Comparator comparator)。
