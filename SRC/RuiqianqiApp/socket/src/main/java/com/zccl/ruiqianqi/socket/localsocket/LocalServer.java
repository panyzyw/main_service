...
public class LocalServer implements Runnable{
    ...
    // 对应的客户端ID
    private Map<String, LocalSocket> mLocalSocketMap;
    // 对应的客户端回调接口
    ...
    private void parseData(String json, byte[] data){
        try {
            ...
            if(LOCAL_LOGIN.equals(cmd)){
                ...
                if (mLocalSocketMap.get(source) != null) {
                    mLocalSocketMap.get(source).close();
                    mLocalSocketMap.remove(source);
                }
                mLocalSocketMap.put(source, localSocketFD.get());
    ...
    public void sendData(String source, String data) {
        try {
            ...
            LocalSocket channel = mLocalSocketMap.get(source);
            if(null == channel)
                return;
    ...
    public void close(){
        try {
            ...
            // 关闭对应客户端
            for(Map.Entry<String, LocalSocket> entry : mLocalSocketMap.entrySet()){
                if(entry.getValue() != null){
