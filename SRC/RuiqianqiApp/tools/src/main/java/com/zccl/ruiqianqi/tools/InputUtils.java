...
public class InputUtils {
    ...
    // 类标志
    private static String TAG = InputUtils.class.getSimpleName();
    // 事件单例
    private static InputUtils instance;
    // 反射的方法句柄
    private Method injectMethod = null;
    // 输入管理器
    private InputManager inputManger = null;

    private InputUtils(Context context) {
        inputManger = (InputManager) context.getSystemService(Context.INPUT_SERVICE);
        try {
            Class[] paramTypes = new Class[2];
            paramTypes[0] = InputEvent.class;
            paramTypes[1] = Integer.TYPE;
            injectMethod = inputManger.getClass().getMethod("injectInputEvent", paramTypes);
    ...
    public static InputUtils getInstance(Context context) {
        if (instance == null) {
            synchronized (InputUtils.class) {
                InputUtils temp = instance;
                if (temp == null) {
                    temp = new InputUtils(context);
                    instance = temp;
                }
            }
        }
...
//        Log.d("jlog", "sendSwipe " + x1 + " " + y1 + " " + x2 + " " + y2 + " " + duration);
...
//        injectMotionEvent(inputSource, MotionEvent.ACTION_UP, now, x2, y2, 0.0f);
    ...
    public void longClickDown(float x, float y) {
        LogUtils.d(TAG, "longClickDown " + x + " " + y);
        sendSwipe(0, x, y, x, y, 500);
    }

    public void longClickUp(float x, float y) {
        LogUtils.d(TAG, "longClickUp " + x + " " + y);
        long now = SystemClock.uptimeMillis();
    ...
    private void injectKeyEvent(KeyEvent event) {
        LogUtils.i(TAG, "injectKeyEvent: " + event);
...
//                InputManager.INJECT_INPUT_EVENT_MODE_WAIT_FOR_FINISH);
    ...
    private void injectMotionEvent(int inputSource, int action, long when, float x, float y, float pressure) {
        ...
        LogUtils.i(TAG, "injectMotionEvent: " + event);
//        InputManager.getInstance().injectInputEvent(event,
//                InputManager.INJECT_INPUT_EVENT_MODE_WAIT_FOR_FINISH);


//        InputManager im = (InputManager) MasterApplication.mAppContext.getSystemService(Context.INPUT_SERVICE);
//        im.injectInputEvent(event, 2);
...
//            hiddenMethod.invoke(im, params);
            if (injectMethod != null) {
                ...
                params[0] = event;
                params[1] = 2;
                injectMethod.invoke(inputManger, params);
            }
